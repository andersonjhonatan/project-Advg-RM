        ...
        <div className="flex gap-4 justify-around">
          {Valores.map((valores: IValores, index) => (
            <div className=" flex flex-col rounded-md py-4 gap-3 items-center text-center p-4 w-full" key={index}>
              <div className="text-4xl text-black">{valores.icon}</div>
              <h1 className={`${roboto.className} text-xl text-black`}>
                {' '}
                {valores.title}
              </h1>
              <p className={`${roboto.className} text-sm text-black whitespace-normal break-all text-justify indent-4` }>{valores.content}</p>
            </div>
          ))}
        </div>
      </div>
